...
Пользователь задаёт должность и вопрос, записывает камеру, микрофон и при необходимости экран либо загружает готовое видео. После обработки он видит видео, транскрипцию, общий балл 0–100 и критерии структуры, ясности, конкретики, релевантности и уверенности, сильные и слабые стороны, слова-паразиты, рекомендации, улучшенную версию ответа и следующий вопрос интервьюера.
...
- фоновые статусы `uploaded`, `transcribing`, `transcription_completed`, `ai_analysis_processing`, `completed`; при сбое Gemini — `ai_analysis_failed`;
...
В desktop-приложении откройте «Настройки → Gemini API», нажмите «Получить Gemini API key», создайте собственный ключ в Google AI Studio и вставьте его в форму. Кнопка «Проверить подключение» выполняет короткий реальный запрос к тому же Gemini endpoint, который использует AI-разбор, но не сохраняет ключ. После проверки нажмите «Сохранить ключ». Валидный ключ попадёт в Keychain на macOS или Credential Locker на Windows. API возвращает только состояние «Gemini подключён/не подключён» и никогда не возвращает сам ключ. Ключ не попадает в HTML, JavaScript, SQLite, логи, установщик или Git.
...
Если ключ отсутствует или Gemini временно недоступен, приложение не падает: транскрипция сохраняется со статусом `transcription_completed` или `ai_analysis_failed`. `completed` появляется только после успешного сохранения проверенного AI-разбора. Кнопка «Повторить AI-анализ» использует сохранённую расшифровку и не запускает Whisper повторно.

Поле ключа — обычное системное password-поле: поддерживаются Cmd/Ctrl+V, Cmd/Ctrl+A, copy/cut/delete и контекстное меню. Ключ должен состоять из ASCII-символов без пробелов; приложение отклоняет некорректно вставленное значение до отправки запроса. На macOS сначала используется `keyring`, а при сбое его backend — системная утилита Keychain; на Windows используется Credential Manager через `keyring`.

Для каждой записи можно выбрать «Русский», «English» или «Авто»; по умолчанию выбран русский. В русской записи Whisper принудительно использует `language="ru"` и `task="transcribe"`, поэтому не переводит ответ в английский. Мультиязычная модель `small` точнее `tiny` для русской речи, но первая загрузка модели и транскрибация на CPU занимают больше времени. Внутри faster-whisper PyAV декодирует медиа в mono/16 kHz; для очень тихой дорожки применяется ограниченная нормализация без повторного lossy-кодирования. Если VAD ошибочно отфильтровал всю речь, приложение повторяет распознавание без VAD. Транскрипция сохраняется в двух вариантах: `raw_transcript` — исходный вывод STT, `clean_transcript` — только нормализованные пробелы. Слова-паразиты, оговорки и повторы не удаляются.

Видео загружается потоково по 1 МБ: оно не удерживается целиком в памяти. Максимум задаётся через `MAX_UPLOAD_SIZE_BYTES` (по умолчанию 500 МБ), а максимальная длительность — через `MAX_VIDEO_DURATION_SECONDS` (60 минут). Desktop-приложение обрабатывает файл локально; публичный лендинг Vercel не является backend-загрузчиком и не принимает видео.
